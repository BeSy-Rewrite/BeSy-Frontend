...
import { BehaviorSubject, forkJoin, from, map, Observable, of, switchMap, tap, throwError } from 'rxjs';
...
export const YEAR_WRAP_GENERATION_MONTH = 11; // December
export const HALF_YEAR_WRAP_GENERATION_MONTH = 0; // June
...
export class UserWrapService {
    ...
    private getOrderSnapshots(): Observable<OrderSnapshot[]> {
        ...
        return this.userService.getCurrentUser().pipe(
            switchMap(user => from(this.cachedOrdersService.getAllOrders(0, 1000, ['lastUpdatedTime,desc'], {
            ...
            map(orderDetailsArray => orderDetailsArray
                .map(orderData => ({
    ...
    private calculateProcessDurationDays(history: OrderStatusHistoryResponseDTO[]): number {
        ...
        return Math.round((new Date(history.at(-1)!.timestamp!).getTime() - new Date(history.at(0)!.timestamp!).getTime()) / (1000 * 60 * 60 * 24)); // days
    }
    ...
    generateWrap(period: WrapPeriod): Observable<UserWrap> {
        ...
        const generationMonth = period === 'year'
    ...
    private getPreviousPeriodWrap(period: WrapPeriod, currentYear: number): UserWrap | undefined {
        ...
        const targetMonth = period === 'year' ? YEAR_WRAP_GENERATION_MONTH : HALF_YEAR_WRAP_GENERATION_MONTH;
        return sorted.find(wrap => {
            const wrapDate = new Date(wrap.generatedAt);
            return wrapDate.getFullYear() < currentYear ||
