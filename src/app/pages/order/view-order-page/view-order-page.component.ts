...
export class ViewOrderPageComponent implements OnInit {
  ...
  export(): void {
    if (!this.internalOrder().order.id) {
      this.snackBar.open(
        'Bestellung hat keine gültige ID und kann nicht exportiert werden.',
        'Schließen',
        { duration: 5000 }
      );
  ...
  changeOrderState(newState: OrderStatus): void {
    if (Date.now() - this.lastStateChangeTimestamp < 1000) {
      this.snackBar.open(
        'Bitte warten Sie einen Moment, bevor Sie den Bestellungsstatus erneut ändern.',
        'Schließen',
        { duration: 5000 }
      );
      ...
      .subscribe({
        next: () => {
          if (newState === OrderStatus.DELETED) {
            ...
            this.ordersService
              .updateOrderState(this.internalOrder().order.id!, newState)
              .then(() => {
                this.snackBar.open(
                  `Bestellungsstatus erfolgreich von \

            '${STATE_DISPLAY_NAMES.get(this.internalOrder().order.status!)}' zu '${STATE_DISPLAY_NAMES.get(newState)}' geändert.`,
                  'Schließen',
                  { duration: 5000 }
                );
